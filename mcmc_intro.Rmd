...
1. The so-called $\hat{R}$ statistic (pronounced R hat) is probably the most widely used diagnostic. It works by running a number of different chains starting at different positions. The idea is that while these chains are all different at the start, they should eventually end up sampling from the same distribution. To work this out, it computes the within-chain variance and compares it to the between-chain variance; if they are very similar, $\hat{R}$ is close to 1, and that's a good sign.
...
Please note that neither of these diagnostics _guarantees_ that everything is OK, as some Bayesians (e.g. Daniel Simpson) are at pains to point out. While that's true and you should always be careful, in my experience $\hat{R}$ in particular is a pretty reliable diagnostic.
...
For the likelihood, we'll use the Bernoulli distribution again. However, the Bernoulli distribution is for binary outcomes, so we could only update using a single coin toss outcome. Instead, we'll be using the Binomial distribution:
...
Check out `Rhat` and `n_eff`, which are the two quantities we mentioned earlier. `Rhat` is 1, and `n_eff` is pretty big (not as big as 4000, the number of samples, but more than big enough), so things look good. Stan also reports the mean and credible intervals for `theta` and `lp__`, whose meaning is a little bit nebulous and not that important (focus on $\theta$).
